...
public class Tests {
    ...
    public void testRandomizedAtoms() {
        ...
        Vector3D[] firstGameRandomAtoms = GamePanel.get().board.trueAtomHexagons;
        GamePanel.get().gameThread.interrupt();

        GamePanel.get().startGameThread();
        assertNotNull(GamePanel.get().board.trueAtomHexagons[0]);
        assertNotNull(GamePanel.get().board.trueAtomHexagons[5]);
        Vector3D[] secondGameRandomAtoms = GamePanel.get().board.trueAtomHexagons;
    ...
    public void testOptions() {
        GamePanel.get().startGameThread();
        OptionsButton button = MainMenuPanel.get().optionsButton;

        GamePanel.get().mouseClicked(new MouseEvent(MainMenuPanel.get(), MouseEvent.MOUSE_CLICKED, 1, 0, (int)button.pos.x + 10, (int)button.pos.y + 10, 1, false));

        assertNotNull(OptionsPanel.get());

        assertNotNull(OptionsPanel.get().muteButton);
        OptionMuteButton button2 = OptionsPanel.get().muteButton;

        button2.performAction();
        assertTrue(OptionsPanel.get().muted);
    }
